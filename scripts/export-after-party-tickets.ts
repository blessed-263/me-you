/**
 * Export After Lunch Party RSVPs for ticket import.
 * Columns: email, ticket type, quantity, name
 *
 * Usage: npm run export:after-party-tickets
 */
import 'dotenv/config';
import { writeFileSync } from 'node:fs';
import { getPool, logDatabaseConfig } from '../server/db.js';

const TICKET_TYPE = 'After Lunch Party - 15h00 - 20:00';
const QUANTITY = 1;
const OUT_FILE = 'after-party-tickets.csv';
...
try {
  const { rows } = await pool.query<{ email: string; full_name: string }>(
    `SELECT email, full_name
     ...
     WHERE session = 'after-party-lunch'
     ORDER BY created_at`,
  );
  ...
  console.error('[export-after-party-tickets] failed:', err);
  process.exit(1);
} finally {
  await pool.end();
}

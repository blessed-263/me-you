/**
 * Bulk-send the Harvest Table day-of email to harvest-table RSVPs.
 *
 * Usage:
 *   npx tsx scripts/send-harvest-table-day-of-from-csv.ts harvest-table-tickets.csv
 */
...
const csvPath = process.argv[2]?.trim() || 'harvest-table-tickets.csv';
...
console.log(`Sending Harvest Table day-of to ${deduped.size} recipients...`);
...
for (const to of deduped) {
  ...
  if (error) {
    console.error(`Failed (Harvest Table day-of → ${to}):`, error);
    process.exit(1);
  }

  console.log(`Sent Harvest Table day-of → ${to} (${data?.id})`);
